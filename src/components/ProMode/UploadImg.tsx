...
const UploadImg: React.FC<UploadImgProps> = ({ onClose, sessionId, onImageSelect }) => {
  const [activeTab, setActiveTab] = useState<'upload using qr' | 'upload from device' | 'yourImages'>('yourImages');
  const [activeSubTab, setActiveSubTab] = useState<'uploaded' | 'generated'>('uploaded'); // New state for sub-tabs
  const [qrCode, setQrCode] = useState<string | null>(null);
  const [images, setImages] = useState<string[]>([]);
  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white rounded-lg p-12 w-full max-w-2xl h-auto shadow-2xl animate-fade-in relative"> {/* Positioned relative for X button */}
        {/* X Button in the top-right corner of the outer div */}
        <button
          onClick={onClose}
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex gap-6">
            <button
              className={`py-4 px-10 rounded-md font-semibold transition-all ${activeTab === 'upload using qr'
                ? 'text-white bg-[#E0AE2A] shadow-md'
                : 'text-[#E0AE2A] border border-[#E0AE2A] hover:bg-[#FDEEC7]'
                }`}
              onClick={() => setActiveTab('upload using qr')}
            ...
            <button
              className={`py-4 px-10 rounded-md font-semibold transition-all ${activeTab === 'upload from device'
                ? 'text-white bg-[#E0AE2A] shadow-md'
                : 'text-[#E0AE2A] border border-[#E0AE2A] hover:bg-[#FDEEC7]'
                }`}
              onClick={() => setActiveTab('upload from device')}
            ...
            <button
              className={`py-4 px-10 rounded-md font-semibold transition-all ${activeTab === 'yourImages'
                ? 'text-white bg-[#E0AE2A] shadow-md'
                : 'text-[#E0AE2A] border border-[#E0AE2A] hover:bg-[#FDEEC7]'
                }`}
              onClick={() => setActiveTab('yourImages')}
