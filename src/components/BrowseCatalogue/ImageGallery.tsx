  ...
  sidebarVisible,
    ...
    return (
      <div className="flex flex-col md:flex-row">
        <div className={`md:block ${sidebarVisible ? 'block' : 'hidden'} w-full md:w-[23%] mt-8`}>
          <FilterSidebar
            filters={filters}
        ...
        <main className="w-full md:w-3/4 p-8 flex flex-col gap-[1.5vh] text-customBlack">
          {paginatedImages.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xs:gap-10 md:gap-12 lg:gap-6 mb-16 mt-8 ">
              {paginatedImages.map((image) => (
                <div
                  key={image.url}
                  className="relative group w-full h-[11rem] md:h-[10rem] lg:h-full "
                ...
                >
                  <div className="relative w-full h-full">
                    ...
                    <LazyLoadImage // Use LazyLoadImage instead of img
                      src={image.url}
                      alt={image.description}
                      className={`w-full h-[11rem] md:h-[10rem] lg:h-full object-cover rounded-lg ${imageLoading[image.url] ? 'opacity-0' : 'opacity-100'}`}
                      placeholder={<FaSpinner />} // Placeholder while loading
                  ...
                  <div className="absolute w-full h-[11rem] md:h-[10rem] lg:h-full inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white p-2 text-center rounded-lg">
                  ...
                  >
                    <AiOutlineHeart
                      size={24}
                      color={likedImages.includes(image.url) ? "red" : "black"}
                    />
                  </button>
                </div>
              ))}
